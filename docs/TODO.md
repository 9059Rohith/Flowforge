...
### P1 — FlowForge Web: the browser fab (serverless, as a static page)
...
(forge push, then a swarm participant). Deploy target: a user-owned static host. Do not
restore the old CNAME unless you control that domain.
...
**Shipped (first milestone, commit 0aa5793):** ops port + mode detection + badge,
  ...
  **Remaining:** (c) Gitea/Forgejo push; (d) enable Pages in repo settings + DNS CNAME
  for the chosen static host; (e) R4: app.js still over budget — split session.
...
- **Generation predicate v0.2 (spec evolution — do not wait on any standards body).**
  ...
  Candidate v0.2 items, drafted the same way as v0.1 (published spec page +
...
- **Browser attest of local files (FlowForge Web).** User points the page at a local folder
