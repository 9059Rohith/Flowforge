"use strict";
// FlowForge web UI — talks to the JSON API in src/server.rs (same `ops` layer as the CLI).
...
async function detectMode() {
  ...
  if (b) {
    ...
    b.title = MODE === "server"
      ? "Running against the local FlowForge server — full sandbox, git forge, all bases"
...
function showBasePrompt(info, onBridged) {
  ...
  p.innerHTML =
    ...
    `<button class="btn small ghost" id="bp-bridge">Use bridged stand-in (FlowForge LLM, not the real base)</button></div>` +
...
function openSandboxedTab(html) {
  ...
  const d = w.document;
  d.open();
  d.write('<!doctype html><meta charset="utf-8"><title>FlowForge — generated app (sandboxed)</title><style>html,body{margin:0;height:100%}iframe{position:fixed;inset:0;width:100%;height:100%;border:0}</style>');
  d.close();
...
function wirePublish() {
  ...
  $("#ghpushgo").onclick = async () => {
    const t = { owner: $("#ghowner").value.trim(), repo: $("#ghrepo").value.trim(), branch: $("#ghbranch").value.trim() || "flowforge", token: $("#ghtoken").value.trim() };
    ...
    try {
      const out = await ForgePush.pushGitHub({ ...t, message: `flowforge: ${STATE.artifacts.run.spec_ref} — code + signed attestation` }, STATE.artifacts);
...
async function runApp() {
  ...
  try {
    ...
    if (r.kind === "web-sandbox") {
      ...
      // Open the running app in a SEPARATE browser tab (keeps the FlowForge tab clean for
...
async function loadAudit(body) {
  ...
  body.appendChild(el("div", "panel-h", "Verify it independently — third-party tools, no FlowForge required:"));
  const tp = el("div", "tp");
